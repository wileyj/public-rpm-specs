...
(
    ...
    find %{buildroot}%{gopath}/src/%{provider}.%{provider_tld}/%{repo_owner}/tools -type d -printf '%%%dir "%p"\n' | %{__sed} -e 's|%{buildroot}||g'
    find %{buildroot}%{gopath}/src/%{provider}.%{provider_tld}/%{repo_owner}/tools -type f -printf '"%p"\n' | %{__sed} -e 's|%{buildroot}||g'
