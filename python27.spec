...
BuildPython optimized python python%{pybasever} "" true
...
InstallPython optimized python%{pybasever} %{py_INSTSONAME_optimized}

for fixed in %{buildroot}%{_bindir}/pydoc; do
    sed 's,#!.*/python$,#!%{_bindir}/env python%{pybasever},' $fixed > $fixed- && cat $fixed- > $fixed && rm -f $fixed-
done
