...
curl -o $RPM_SOURCE_DIR/%{name}.tar.gz `curl -s https://pypi.python.org/pypi/%{pkgname}-python | grep tar.gz | cut -d '"' -f2 | cut -f1 -d "#" | tail -1`
tar -xzvf $RPM_SOURCE_DIR/%{name}.tar.gz
mv %{_builddir}/%{pkgname}-python-%{version} %{_builddir}/%{name}-%{version}
