...
%define pypi_name crontab
%define pypi_alternate python-crontab

%global pypi_version_test %(echo `curl -s https://pypi.python.org/pypi/%{pypi_alternate} | grep "<title>" | awk '{print $2}'`)
%if "%{?pypi_version_test:%{pypi_version_test}}%{!?pypi_version_test:0}" == "of"
%global pypi_version %(echo `curl -s https://pypi.python.org/pypi/%{pypi_alternate} | sed -n -e '/<table class="list">/{n;n;n;n;n;n;n;n;p;};h' | cut -d'"' -f2 | cut -d'/' -f4`)
%global pypi_url https://pypi.python.org/pypi/%{pypi_alternate}/%{pypi_version}
%define multi true
%else
%global pypi_version %{pypi_version_test}
%global pypi_url https://pypi.python.org/pypi/%{pypi_alternate}
%define multi false
%endif
...
#mv %{_builddir}/%{pypi_alternate}-%{version} %{_builddir}/%{name}-%{version}
