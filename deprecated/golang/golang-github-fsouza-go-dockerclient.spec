...
! -path %{buildroot}%{gopath}/pkg/linux_amd64/%{provider}.%{provider_tld}/opencontainers \
...
! -path %{buildroot}%{gopath}/src/%{provider}.%{provider_tld}/opencontainers \
! -path %{buildroot}%{gopath}/src/%{provider}.%{provider_tld}/%{repo_owner}`; do
    %__rm -rf ${src_dir}
done

for pkg_dir in `find %{buildroot}%{gopath}/pkg/linux_amd64/%{provider}.%{provider_tld}/opencontainers -maxdepth 1 \
! -path %{buildroot}%{gopath}/pkg/linux_amd64/%{provider}.%{provider_tld}/opencontainers/runc`; do
    %__rm -rf ${src_dir}
done

for src_dir in `find %{buildroot}%{gopath}/src/%{provider}.%{provider_tld}/opencontainers -maxdepth 1 \
! -path %{buildroot}%{gopath}/src/%{provider}.%{provider_tld}/opencontainers/runc`; do
    %__rm -rf ${src_dir}
done
