%define repo https://github.com/masterzen/winrm
%global provider        github
%global provider_tld    com
%global repo_owner      masterzen
%global project         winrm
...
%{__rm} -rf %{buildroot}%{gopath}/src/%{provider}.%{provider_tld}/masterzen/xmlpath
%{__rm} -rf %{buildroot}%{gopath}/src/%{provider}.%{provider_tld}/masterzen/simplexml
%{__rm} -rf %{buildroot}%{gopath}/src/%{provider}.%{provider_tld}/nu7hatch
%{__rm} -rf %{buildroot}%{gopath}/pkg/linux_amd64/%{provider}.%{provider_tld}/nu7hatch
(
...
echo '%{gopath}/bin/winrm' >> %{name}-%{version}-filelist
