...
Requires: rubygem-sensu-extension
Requires: rubygem-sensu-extensions-json
Requires: rubygem-sensu-extensions-debug
Requires: rubygem-sensu-extensions-occurrences
Requires: rubygem-sensu-extensions-only-check-output
Requires: rubygem-sensu-extensions-ruby-hash
...
if [ -f %{buildroot}%{gem_dir}/gems/%{gem_name}-%{version}/lib/sensu/extensions/loader.rb ]
then
    echo "Making changes using sed to %{buildroot}%{gem_dir}/gems/%{gem_name}-%{version}/lib/sensu/extensions/loader.rb"
    sed -i -e 's|", "[0-9].*|"|g'  %{buildroot}%{gem_dir}/gems/%{gem_name}-%{version}/lib/sensu/extensions/loader.rb
fi
