...
BuildRequires: ruby-rpm-macros rubygem-rpm-macros
...
Requires:   rubygem-rpm-macros
Requires:   ruby-rpm-macros
...
##sh %{SOURCE14} -f %{buildroot}%{_libdir}/libruby.so.%{ruby_version} | grep "Full RELRO.*Canary found.*NX enabled.*DSO.*No RPATH.*No RUNPATH.*Yes.*\d*.*\d*.*libruby.so.%{ruby_version}"
...
%exclude %{ruby_libarchdir}/openssl*
%exclude %{_libdir}/gems/%{name}/openssl*
%exclude %{gem_dir}/gems/openssl*
%exclude %{rubygems_dir}/rubygems/defaults/openssl*
