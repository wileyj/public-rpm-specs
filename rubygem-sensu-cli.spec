...
then
    ...
    %{__sed} -i -e "s|('rainbow.*| \"rainbow\"|g" %{buildroot}%{gem_dir}/gems/%{gemname}-%{version}/%{gemname}.gemspec
    %{__sed} -i -e "s|('trollop.*| \"trollop\"|g" %{buildroot}%{gem_dir}/gems/%{gemname}-%{version}/%{gemname}.gemspec
    %{__sed} -i -e "s|('hirb.*| \"hirb\"|g" %{buildroot}%{gem_dir}/gems/%{gemname}-%{version}/%{gemname}.gemspec
