...
Requires:       git = %{version}-%{release}, perl-Error, perl-TermReadKey
...
find -name "*.py" -exec \
  sed \
    -i -e "s|/usr/bin/env python|%{__python27}|" \
    "{}" \
    \;
...
test ! -d $RPM_BUILD_ROOT%{python27_sitelib} || rm -fr $RPM_BUILD_ROOT%{python27_sitelib}
