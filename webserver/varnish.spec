...
%define varnish_prefix /opt/%{name}
...
%if 0%{?with_python3}
BuildRequires: python3-docutils >= 0.6
...
%configure  --without-rst2html \
	--localstatedir=/var/lib \
	--prefix=%{varnish_prefix} \
  	--bindir=%{_bindir} \
  	--sbindir=%{_sbindir} \
  	--libexecdir=%{varnish_prefix}/libexec \
  	--sysconfdir=%{varnish_prefix}/etc \
  	--libdir=%{_libdir} \
...
install -D -m 0644 etc/example.vcl %{buildroot}%{varnish_prefix}/etc/default.vcl
install -D -m 0644 varnish.logrotate %{buildroot}%{_sysconfdir}/logrotate.d/varnish
%__ln_s %{varnish_prefix}/etc %{buildroot}%{_sysconfdir}/%{name}
...
install -D -m 0644 varnish.params %{buildroot}%{varnish_prefix}/etc/varnish.params
...
%{_mandir}/man3/*.3*
%{_mandir}/man7/*.7*
#%{_docdir}/varnish-%{version}
%doc doc/html
%doc doc/changes*.html
#%dir %{_sysconfdir}/varnish/
#%{_sysconfdir}/varnish/
%config(noreplace) %{varnish_prefix}/etc/default.vcl
#%config(noreplace) %{_sysconfdir}/varnish/default.vcl
%config(noreplace) %{_sysconfdir}/logrotate.d/varnish
%{varnish_prefix}/share/doc/%{name}/builtin.vcl
%{varnish_prefix}/share/doc/%{name}/example.vcl
%{_sysconfdir}/%{name}
...
%config(noreplace)%{varnish_prefix}/etc/varnish.params
