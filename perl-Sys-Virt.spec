...
Patch1:         %{modname}-%{version}-cleanup-leaks.patch
Patch2:         %{modname}-%{version}-remove-imports.patch
Patch3:         %{modname}-%{version}-export-error-consts.patch
Patch4:         %{modname}-%{version}-undefined-string.patch
Patch5:         %{modname}-%{version}-default-values.patch
Patch6:         %{modname}-%{version}-undefined-objects.patch
Patch7:         %{modname}-%{version}-uuid-buflen.patch
BuildRoot:      %{_tmppath}/%{name}-%{version}-%{release}-root-%(%{__id_u} -n)
BuildRequires:  perl(ExtUtils::MakeMaker)
BuildRequires: make, gcc, perl-srpm-macros
