%define cpan_name DBIx::DWIW
%define pkgname DBIx-DWIW
%define cpan_version %(echo `curl -s https://metacpan.org/pod/%{cpan_name} | grep "Module version" | cut -d":" -f2`)
%define filelist %{pkgname}-%{version}-filelist

name:      perl-DBIx-DWIW
summary:   DBIx-DWIW - Robust and simple DBI wrapper to Do What I Want (DWIW)
...
prefix:    %(echo %{_prefix})
BuildRequires: perl-DBI, perl
BuildRequires: perl, perl-srpm-macros,  perl-devel, perl-libs, perl-ExtUtils-MakeMaker, perl-Time-HiRes
...
This module was recently extracted from Yahoo-specific code, so things
may be a little strange yet while we smooth out any bumps and blemishes
left over form that.

DBIx::DWIW is intended to be sub-classed. Doing so gives you all the
...
"examples/MyDBI.pm" which illustrates some of what you might want to do in
your own class(es).
