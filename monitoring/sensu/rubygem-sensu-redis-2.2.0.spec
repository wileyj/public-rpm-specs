...
Name: rubygem-sensu-redis
Version: 2.2.0
Release: 1.%{dist}
Summary: rubygem sensu-redis
...
Provides: rubygem-sensu-redis = %{version}
Provides: rubygem(sensu-redis) = %{version}
Obsoletes: rubygem-sensu-redis < %{version}
Obsoletes: rubygem(sensu-redis) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-redis -v %{version}
