...
Provides: rubygem-sensu-extensions = %{version}
Provides: rubygem(sensu-extensions) = %{version}
Obsoletes: rubygem-sensu-extensions < %{version}
Obsoletes: rubygem(sensu-extensions) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-extensions -v %{version}
