...
%global gem_name sensu-plugins-erlang
Name: rubygem-sensu-plugins-erlang
Version: 1.0.0
Release: 1.%{dist}
Summary: rubygem sensu-plugins-erlang
Group: Development/Languages
License: MIT
URL: https://github.com/sensu-plugins/sensu-plugins-erlang
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-sensu-plugins-erlang = %{version}
Provides: rubygem(sensu-plugins-erlang) = %{version}
Obsoletes: rubygem-sensu-plugins-erlang < %{version}
Obsoletes: rubygem(sensu-plugins-erlang) < %{version}
...
%description
Sensu plugins for erlang

%prep
%build
%install
export CONFIGURE_ARGS="--with-cflags='%{optflags}'"
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-erlang -v %{version}
