...
Name: rubygem-sensu-logger
Version: 1.2.1
Release: 1.%{dist}
Summary: rubygem sensu-logger
...
Provides: rubygem-sensu-logger = %{version}
Provides: rubygem(sensu-logger) = %{version}
Obsoletes: rubygem-sensu-logger < %{version}
Obsoletes: rubygem(sensu-logger) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-logger -v %{version}
