...
Name: rubygem-sensu-extensions-debug
Version: 1.0.0
Release: 1.%{dist}
Summary: rubygem sensu-extensions-debug
...
Provides: rubygem-sensu-extensions-debug = %{version}
Provides: rubygem(sensu-extensions-debug) = %{version}
Obsoletes: rubygem-sensu-extensions-debug < %{version}
Obsoletes: rubygem(sensu-extensions-debug) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-extensions-debug -v %{version}
