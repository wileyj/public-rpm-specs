...
Name: rubygem-sensu-extensions-only-check-output
Version: 1.0.0
Release: 1.%{dist}
Summary: rubygem sensu-extensions-only-check-output
Group: Development/Languages
License: GPL
URL: https://github.com/sensu-extensions/sensu-extensions-only-check-output
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-sensu-extensions-only-check-output = %{version}
Provides: rubygem(sensu-extensions-only-check-output) = %{version}
Obsoletes: rubygem-sensu-extensions-only-check-output < %{version}
Obsoletes: rubygem(sensu-extensions-only-check-output) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-extensions-only-check-output -v %{version}
