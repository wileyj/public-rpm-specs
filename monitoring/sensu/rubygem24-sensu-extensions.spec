...
Name: rubygem%{alinux_ruby}-sensu-extensions
Version: 1.9.0
Release: 1.%{dist}
AutoReqProv : no
Summary: rubygem sensu-extensions
...
Provides: rubygem%{alinux_ruby}-sensu-extensions = %{version}
Provides: rubygem%{alinux_ruby}(sensu-extensions) = %{version}
Obsoletes: rubygem%{alinux_ruby}-sensu-extensions < %{version}
Obsoletes: rubygem%{alinux_ruby}(sensu-extensions) < %{version}


Requires: rubygem%{alinux_ruby}-sensu-extension
Requires: rubygem%{alinux_ruby}-sensu-extensions-check-dependencies  >= 1.0.1
Requires: rubygem%{alinux_ruby}-sensu-extensions-debug  >= 1.0.0
Requires: rubygem%{alinux_ruby}-sensu-extensions-json  >= 1.0.0
Requires: rubygem%{alinux_ruby}-sensu-extensions-occurrences  >= 1.2.0
Requires: rubygem%{alinux_ruby}-sensu-extensions-only-check-output  >= 1.0.0
Requires: rubygem%{alinux_ruby}-sensu-extensions-ruby-hash  >= 1.0.0
Requires: rubygem%{alinux_ruby}-sensu-json  >= 1.1.0
Requires: rubygem%{alinux_ruby}-sensu-logger
Requires: rubygem%{alinux_ruby}-sensu-settings
