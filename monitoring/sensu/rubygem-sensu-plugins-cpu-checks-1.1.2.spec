...
%global gem_name sensu-plugins-cpu-checks
Name: rubygem-sensu-plugins-cpu-checks
Version: 1.1.2
Release: 1.%{dist}
Summary: rubygem sensu-plugins-cpu-checks
Group: Development/Languages
License: MIT
URL: https://github.com/sensu-plugins/sensu-plugins-cpu-checks
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-sensu-plugins-cpu-checks = %{version}
Provides: rubygem(sensu-plugins-cpu-checks) = %{version}
Obsoletes: rubygem-sensu-plugins-cpu-checks < %{version}
Obsoletes: rubygem(sensu-plugins-cpu-checks) < %{version}


Requires: rubygem-linux-kstat  = 0.1.3
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-cpu-checks -v %{version}
