...
%global gem_name sensu-plugins-network-checks
Name: rubygem-sensu-plugins-network-checks
Version: 2.1.0
Release: 1.%{dist}
Summary: rubygem sensu-plugins-network-checks
Group: Development/Languages
License: MIT
URL: https://github.com/sensu-plugins/sensu-plugins-network-checks
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-sensu-plugins-network-checks = %{version}
Provides: rubygem(sensu-plugins-network-checks) = %{version}
Obsoletes: rubygem-sensu-plugins-network-checks < %{version}
Obsoletes: rubygem(sensu-plugins-network-checks) < %{version}


Requires: rubygem-activesupport  >= 4.2
Requires: rubygem-dnsbl-client  = 1.0.2
Requires: rubygem-net-ping  = 1.7.8
Requires: rubygem-sensu-plugin  >= 1.2
Requires: rubygem-whois  >= 4.0
Requires: rubygem-whois-parser  = 1.0.0
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-network-checks -v %{version}
