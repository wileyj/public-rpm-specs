...
%global gem_name sensu-plugins-memory-checks
Name: rubygem-sensu-plugins-memory-checks
Version: 3.1.1
Release: 1.%{dist}
Summary: rubygem sensu-plugins-memory-checks
Group: Development/Languages
License: MIT
URL: https://github.com/sensu-plugins/sensu-plugins-memory-checks
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-sensu-plugins-memory-checks = %{version}
Provides: rubygem(sensu-plugins-memory-checks) = %{version}
Obsoletes: rubygem-sensu-plugins-memory-checks < %{version}
Obsoletes: rubygem(sensu-plugins-memory-checks) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-memory-checks -v %{version}
