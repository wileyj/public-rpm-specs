...
%global gem_name sensu-plugins-uptime
Name: rubygem-sensu-plugins-uptime
Version: 0.0.1
Release: 1.%{dist}
Summary: rubygem sensu-plugins-uptime
Group: Development/Languages
License: MIT
URL: https://github.com/m4ce/sensu-plugins-uptime
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-sensu-plugins-uptime = %{version}
Provides: rubygem(sensu-plugins-uptime) = %{version}
Obsoletes: rubygem-sensu-plugins-uptime < %{version}
Obsoletes: rubygem(sensu-plugins-uptime) < %{version}


Requires: rubygem-sensu-plugin  >= 1.2.0


%description
This plugin provides facilities for monitoring the system uptime

%prep
%build
%install
export CONFIGURE_ARGS="--with-cflags='%{optflags}'"
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-uptime -v %{version}
