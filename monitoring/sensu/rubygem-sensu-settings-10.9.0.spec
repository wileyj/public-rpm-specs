...
Name: rubygem-sensu-settings
Version: 10.9.0
Release: 1.%{dist}
Summary: rubygem sensu-settings
...
Provides: rubygem-sensu-settings = %{version}
Provides: rubygem(sensu-settings) = %{version}
Obsoletes: rubygem-sensu-settings < %{version}
Obsoletes: rubygem(sensu-settings) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-settings -v %{version}
