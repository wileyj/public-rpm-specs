...
%global gem_name sensu-plugins-docker
Name: rubygem-sensu-plugins-docker
Version: 2.0.0
Release: 1.%{dist}
Summary: rubygem sensu-plugins-docker
...
Provides: rubygem-sensu-plugins-docker = %{version}
Provides: rubygem(sensu-plugins-docker) = %{version}
Obsoletes: rubygem-sensu-plugins-docker < %{version}
Obsoletes: rubygem(sensu-plugins-docker) < %{version}


Requires: rubygem-docker-api  = 1.21.0
Requires: rubygem-net_http_unix  = 0.2.2
Requires: rubygem-sensu-plugin  >= 2.0
Requires: rubygem-sys-proctable  = 0.9.8
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-docker -v %{version}
