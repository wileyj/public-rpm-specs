...
%global gem_name sensu-plugins-sensu
Name: rubygem-sensu-plugins-sensu
Version: 2.4.0
Release: 1.%{dist}
Summary: rubygem sensu-plugins-sensu
...
Provides: rubygem-sensu-plugins-sensu = %{version}
Provides: rubygem(sensu-plugins-sensu) = %{version}
Obsoletes: rubygem-sensu-plugins-sensu < %{version}
Obsoletes: rubygem(sensu-plugins-sensu) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-sensu -v %{version}
