...
Name: rubygem-sensu-spawn
Version: 2.2.1
Release: 1.%{dist}
Summary: rubygem sensu-spawn
...
Provides: rubygem-sensu-spawn = %{version}
Provides: rubygem(sensu-spawn) = %{version}
Obsoletes: rubygem-sensu-spawn < %{version}
Obsoletes: rubygem(sensu-spawn) < %{version}


Requires: rubygem-childprocess  = 0.5.8
Requires: rubygem-em-worker  = 0.0.2
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-spawn -v %{version}
