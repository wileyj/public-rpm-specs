...
%global gem_name sensu-plugins-golang
Name: rubygem-sensu-plugins-golang
Version: 1.0.0
Release: 1.%{dist}
Summary: rubygem sensu-plugins-golang
Group: Development/Languages
License: MIT
URL: https://github.com/sensu-plugins/sensu-plugins-golang
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-sensu-plugins-golang = %{version}
Provides: rubygem(sensu-plugins-golang) = %{version}
Obsoletes: rubygem-sensu-plugins-golang < %{version}
Obsoletes: rubygem(sensu-plugins-golang) < %{version}


Requires: rubygem-oj  = 2.18.1
Requires: rubygem-sensu-plugin  >= 1.2


%description
Sensu plugins for working with golang

%prep
%build
%install
export CONFIGURE_ARGS="--with-cflags='%{optflags}'"
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-golang -v %{version}
