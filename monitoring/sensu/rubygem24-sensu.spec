...
Name: rubygem%{alinux_ruby}-sensu
Version: 1.2.0
Release: 1.%{dist}
AutoReqProv : no
Summary: rubygem sensu
...
Provides: rubygem%{alinux_ruby}-sensu = %{version}
Provides: rubygem%{alinux_ruby}(sensu) = %{version}
Obsoletes: rubygem%{alinux_ruby}-sensu < %{version}
Obsoletes: rubygem%{alinux_ruby}(sensu) < %{version}


Requires: rubygem%{alinux_ruby}-em-http-server  >= 0.1.8
Requires: rubygem%{alinux_ruby}-eventmachine  >= 1.2.5
Requires: rubygem%{alinux_ruby}-parse-cron  >= 0.1.4
Requires: rubygem%{alinux_ruby}-sensu-extension  >= 1.5.1
Requires: rubygem%{alinux_ruby}-sensu-extensions  >= 1.9.0
Requires: rubygem%{alinux_ruby}-sensu-json  >= 2.1.0
Requires: rubygem%{alinux_ruby}-sensu-logger  >= 1.2.1
Requires: rubygem%{alinux_ruby}-sensu-redis  >= 2.2.0
Requires: rubygem%{alinux_ruby}-sensu-settings  >= 10.12.0
Requires: rubygem%{alinux_ruby}-sensu-spawn  >= 2.2.1
Requires: rubygem%{alinux_ruby}-sensu-transport  >= 7.0.2
