...
Name: rubygem-sensu-json
Version: 2.1.0
Release: 1.%{dist}
Summary: rubygem sensu-json
...
Provides: rubygem-sensu-json = %{version}
Provides: rubygem(sensu-json) = %{version}
Obsoletes: rubygem-sensu-json < %{version}
Obsoletes: rubygem(sensu-json) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-json -v %{version}
