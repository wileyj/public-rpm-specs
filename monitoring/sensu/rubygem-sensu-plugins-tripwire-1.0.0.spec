...
%global gem_name sensu-plugins-tripwire
Name: rubygem-sensu-plugins-tripwire
Version: 1.0.0
Release: 1.%{dist}
Summary: rubygem sensu-plugins-tripwire
Group: Development/Languages
License: MIT
URL: https://github.com/sensu-plugins/sensu-plugins-tripwire
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-sensu-plugins-tripwire = %{version}
Provides: rubygem(sensu-plugins-tripwire) = %{version}
Obsoletes: rubygem-sensu-plugins-tripwire < %{version}
Obsoletes: rubygem(sensu-plugins-tripwire) < %{version}
...
%description
Sensu plugins for tripwire

%prep
%build
%install
export CONFIGURE_ARGS="--with-cflags='%{optflags}'"
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-tripwire -v %{version}
