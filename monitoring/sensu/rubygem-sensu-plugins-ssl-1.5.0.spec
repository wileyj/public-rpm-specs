...
%global gem_name sensu-plugins-ssl
Name: rubygem-sensu-plugins-ssl
Version: 1.5.0
Release: 1.%{dist}
Summary: rubygem sensu-plugins-ssl
...
Provides: rubygem-sensu-plugins-ssl = %{version}
Provides: rubygem(sensu-plugins-ssl) = %{version}
Obsoletes: rubygem-sensu-plugins-ssl < %{version}
Obsoletes: rubygem(sensu-plugins-ssl) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-ssl -v %{version}
