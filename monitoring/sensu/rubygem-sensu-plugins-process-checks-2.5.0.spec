...
Name: rubygem-sensu-plugins-process-checks
Version: 2.5.0
Release: 1.%{dist}
...
Provides: rubygem-sensu-plugins-process-checks = %{version}
Provides: rubygem(sensu-plugins-process-checks) = %{version}
Obsoletes: rubygem-sensu-plugins-process-checks < %{version}
Obsoletes: rubygem(sensu-plugins-process-checks) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-process-checks -v %{version}
