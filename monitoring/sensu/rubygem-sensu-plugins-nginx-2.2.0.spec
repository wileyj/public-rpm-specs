...
%global gem_name sensu-plugins-nginx
Name: rubygem-sensu-plugins-nginx
Version: 2.2.0
Release: 1.%{dist}
Summary: rubygem sensu-plugins-nginx
Group: Development/Languages
License: MIT
URL: https://github.com/sensu-plugins/sensu-plugins-nginx
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-sensu-plugins-nginx = %{version}
Provides: rubygem(sensu-plugins-nginx) = %{version}
Obsoletes: rubygem-sensu-plugins-nginx < %{version}
Obsoletes: rubygem(sensu-plugins-nginx) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-nginx -v %{version}
