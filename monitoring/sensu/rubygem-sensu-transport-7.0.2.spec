...
Provides: rubygem-sensu-transport = %{version}
Provides: rubygem(sensu-transport) = %{version}
Obsoletes: rubygem-sensu-transport < %{version}
Obsoletes: rubygem(sensu-transport) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-transport -v %{version}
