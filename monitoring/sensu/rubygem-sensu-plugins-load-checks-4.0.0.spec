...
%global gem_name sensu-plugins-load-checks
Name: rubygem-sensu-plugins-load-checks
Version: 4.0.0
Release: 1.%{dist}
Summary: rubygem sensu-plugins-load-checks
Group: Development/Languages
License: MIT
URL: https://github.com/sensu-plugins/sensu-plugins-load-checks
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-sensu-plugins-load-checks = %{version}
Provides: rubygem(sensu-plugins-load-checks) = %{version}
Obsoletes: rubygem-sensu-plugins-load-checks < %{version}
Obsoletes: rubygem(sensu-plugins-load-checks) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-load-checks -v %{version}
