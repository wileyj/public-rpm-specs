...
Provides: rubygem-sensu = %{version}
Provides: rubygem(sensu) = %{version}
Obsoletes: rubygem-sensu < %{version}
Obsoletes: rubygem(sensu) < %{version}


Requires: rubygem-em-http-server  = 0.1.8
Requires: rubygem-eventmachine  = 1.2.5
Requires: rubygem-parse-cron  = 0.1.4
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu -v %{version}
