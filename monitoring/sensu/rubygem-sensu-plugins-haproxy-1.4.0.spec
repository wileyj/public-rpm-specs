...
%global gem_name sensu-plugins-haproxy
Name: rubygem-sensu-plugins-haproxy
Version: 1.4.0
Release: 1.%{dist}
Summary: rubygem sensu-plugins-haproxy
Group: Development/Languages
License: MIT
URL: https://github.com/sensu-plugins/sensu-plugins-haproxy
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-sensu-plugins-haproxy = %{version}
Provides: rubygem(sensu-plugins-haproxy) = %{version}
Obsoletes: rubygem-sensu-plugins-haproxy < %{version}
Obsoletes: rubygem(sensu-plugins-haproxy) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-haproxy -v %{version}
