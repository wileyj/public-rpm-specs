...
Name: rubygem-sensu-plugin
Version: 2.3.0
Release: 1.%{dist}
Summary: rubygem sensu-plugin
Group: Development/Languages
License: MIT
URL: https://github.com/sensu-plugins/sensu-plugin
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-sensu-plugin = %{version}
Provides: rubygem(sensu-plugin) = %{version}
Obsoletes: rubygem-sensu-plugin < %{version}
Obsoletes: rubygem(sensu-plugin) < %{version}


Requires: rubygem-json  < 3.0.0
Requires: rubygem-mixlib-cli  >= 1.5.0
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugin -v %{version}
