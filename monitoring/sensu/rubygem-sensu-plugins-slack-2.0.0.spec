...
%global gem_name sensu-plugins-slack
Name: rubygem-sensu-plugins-slack
Version: 2.0.0
Release: 1.%{dist}
Summary: rubygem sensu-plugins-slack
...
Provides: rubygem-sensu-plugins-slack = %{version}
Provides: rubygem(sensu-plugins-slack) = %{version}
Obsoletes: rubygem-sensu-plugins-slack < %{version}
Obsoletes: rubygem(sensu-plugins-slack) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sensu-plugins-slack -v %{version}
