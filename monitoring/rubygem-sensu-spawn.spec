...
then
    ...
    sed -i -e 's|"childprocess".*|"childprocess"|g' %{buildroot}%{gem_dir}/gems/%{gemname}-%{version}/%{gemname}.gemspec
    sed -i -e 's|"em-worker".*|"em-worker"|g' %{buildroot}%{gem_dir}/gems/%{gemname}-%{version}/%{gemname}.gemspec
fi
if [ -f %{buildroot}%{gem_dir}/gems/%{gemname}-%{version}/lib/sensu/spawn.rb ]
then
	sed -i -e 's|"childprocess".*|"childprocess"|g' %{buildroot}%{gem_dir}/gems/%{gemname}-%{version}/lib/sensu/spawn.rb
	sed -i -e 's|"em-worker".*|"em-worker"|g' %{buildroot}%{gem_dir}/gems/%{gemname}-%{version}/lib/sensu/spawn.rb
fi
