...
%global repo_owner      ugorji
%global project         go
%global import_path     %{provider}.%{provider_tld}/%{repo_owner}/%{project}
%define _summary        Package that provides easy configuration management for Go programs
...
cp -pav codec/* %{buildroot}/%{gopath}/src/%{import_path}/
...
echo '"%{gopath}/src/%{provider}.%{provider_tld}/%{repo_owner}/%{project}/test.pyc"' >> filelist
echo '"%{gopath}/src/%{provider}.%{provider_tld}/%{repo_owner}/%{project}/test.pyo"' >> filelist
