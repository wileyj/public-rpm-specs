...
curl -o $RPM_SOURCE_DIR/%{name}.tar.gz `curl -s https://pypi.python.org/pypi/%{pkgname} | grep ".tar.gz" | cut -d '"' -f2 | cut -f1 -d "#" | tail -1`
...
sed -i -e 's|#!/usr/bin/python|#!/usr/bin/%{pexec}|g' $RPM_BUILD_ROOT/usr/bin/lss3 
sed -i -e 's|#!/usr/bin/python|#!/usr/bin/%{pexec}|g' $RPM_BUILD_ROOT/usr/bin/route53
