...
Provides: rubygem(mixlib-shellout) = %{version}
Obsoletes: rubygem-mixlib-shellout < %{version}
Obsoletes: rubygem(mixlib-shellout) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies mixlib-shellout -v %{version}
