...
Provides: rubygem-kubeclient = %{version}
Provides: rubygem(kubeclient) = %{version}
Obsoletes: rubygem-kubeclient < %{version}
Obsoletes: rubygem(kubeclient) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies kubeclient -v %{version}
