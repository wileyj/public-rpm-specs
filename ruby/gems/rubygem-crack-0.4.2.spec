...
Provides: rubygem-crack = %{version}
Provides: rubygem(crack) = %{version}
Obsoletes: rubygem-crack < %{version}
Obsoletes: rubygem(crack) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies crack -v %{version}
