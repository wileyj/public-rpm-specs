...
Provides: rubygem-bson = %{version}
Provides: rubygem(bson) = %{version}
Obsoletes: rubygem-bson < %{version}
Obsoletes: rubygem(bson) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies bson -v %{version}
