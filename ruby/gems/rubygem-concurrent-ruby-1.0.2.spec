...
Provides: rubygem-concurrent-ruby = %{version}
Provides: rubygem(concurrent-ruby) = %{version}
Obsoletes: rubygem-concurrent-ruby < %{version}
Obsoletes: rubygem(concurrent-ruby) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies concurrent-ruby -v %{version}
