...
Provides: rubygem(mail) = %{version}
Obsoletes: rubygem-mail < %{version}
Obsoletes: rubygem(mail) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies mail -v %{version}
