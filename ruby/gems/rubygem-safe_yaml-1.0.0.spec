...
Provides: rubygem-safe_yaml = %{version}
Provides: rubygem(safe_yaml) = %{version}
Obsoletes: rubygem-safe_yaml < %{version}
Obsoletes: rubygem(safe_yaml) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies safe_yaml -v %{version}
