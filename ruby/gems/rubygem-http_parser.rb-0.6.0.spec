...
Provides: rubygem(http_parser.rb) = %{version}
Obsoletes: rubygem-http_parser.rb < %{version}
Obsoletes: rubygem(http_parser.rb) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies http_parser.rb -v %{version}
