...
Provides: rubygem-netrc = %{version}
Provides: rubygem(netrc) = %{version}
Obsoletes: rubygem-netrc < %{version}
Obsoletes: rubygem(netrc) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies netrc -v %{version}
