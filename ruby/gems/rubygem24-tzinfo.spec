...
Name: rubygem%{alinux_ruby}-tzinfo
Version: 1.2.4
Release: 1.%{dist}
AutoReqProv : no
...
Provides: rubygem%{alinux_ruby}-tzinfo = %{version}
Provides: rubygem%{alinux_ruby}(tzinfo) = %{version}
Obsoletes: rubygem%{alinux_ruby}-tzinfo < %{version}
Obsoletes: rubygem%{alinux_ruby}(tzinfo) < %{version}


Requires: rubygem%{alinux_ruby}-thread_safe  > 0.1
