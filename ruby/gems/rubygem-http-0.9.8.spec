...
Provides: rubygem-http = %{version}
Provides: rubygem(http) = %{version}
Obsoletes: rubygem-http < %{version}
Obsoletes: rubygem(http) < %{version}


Requires: rubygem-addressable  >= 2.3
Requires: rubygem-http-cookie  >= 1.0
Requires: rubygem-http-form_data  >= 1.0.1
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies http -v %{version}
