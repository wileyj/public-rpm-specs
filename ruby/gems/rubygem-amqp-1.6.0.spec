...
Provides: rubygem-amqp = %{version}
Provides: rubygem(amqp) = %{version}
Obsoletes: rubygem-amqp < %{version}
Obsoletes: rubygem(amqp) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies amqp -v %{version}
