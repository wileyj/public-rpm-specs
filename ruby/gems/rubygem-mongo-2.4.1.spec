...
Provides: rubygem-mongo = %{version}
Provides: rubygem(mongo) = %{version}
Obsoletes: rubygem-mongo < %{version}
Obsoletes: rubygem(mongo) < %{version}


Requires: rubygem-bson  < 5.0.0,
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies mongo -v %{version}
