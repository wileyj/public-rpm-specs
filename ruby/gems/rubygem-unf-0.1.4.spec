...
Provides: rubygem-unf = %{version}
Provides: rubygem(unf) = %{version}
Obsoletes: rubygem-unf < %{version}
Obsoletes: rubygem(unf) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies unf -v %{version}
