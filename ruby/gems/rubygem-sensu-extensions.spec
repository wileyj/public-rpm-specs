...
then
    ...
    sed -i -e '\|sensu-extensions-debug|d' %{buildroot}%{gem_dir}/specifications/%{gem_name}-%{version}.gemspec
fi
...
then
    ...
    sed -i -e '\|sensu-extensions-debug|d'  %{buildroot}%{gem_dir}/gems/%{gem_name}-%{version}/%{gem_name}.gemspec
fi
