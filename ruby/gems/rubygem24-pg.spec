...
Provides: rubygem%{alinux_ruby}-pg = %{version}
Provides: rubygem%{alinux_ruby}(pg) = %{version}
Obsoletes: rubygem%{alinux_ruby}-pg < %{version}
Obsoletes: rubygem%{alinux_ruby}(pg) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --no-rdoc --no-ri --no-doc --ignore-dependencies pg -v %{version}
