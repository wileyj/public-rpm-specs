...
Provides: rubygem-public_suffix = %{version}
Provides: rubygem(public_suffix) = %{version}
Obsoletes: rubygem-public_suffix < %{version}
Obsoletes: rubygem(public_suffix) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies public_suffix -v %{version}
