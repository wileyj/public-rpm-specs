...
Provides: rubygem-childprocess = %{version}
Provides: rubygem(childprocess) = %{version}
Obsoletes: rubygem-childprocess < %{version}
Obsoletes: rubygem(childprocess) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies childprocess -v %{version}
