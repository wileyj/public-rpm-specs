...
Provides: rubygem-amq-protocol = %{version}
Provides: rubygem(amq-protocol) = %{version}
Obsoletes: rubygem-amq-protocol < %{version}
Obsoletes: rubygem(amq-protocol) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies amq-protocol -v %{version}
