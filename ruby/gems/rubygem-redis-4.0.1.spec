...
Provides: rubygem-redis = %{version}
Provides: rubygem(redis) = %{version}
Obsoletes: rubygem-redis < %{version}
Obsoletes: rubygem(redis) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies redis -v %{version}
