...
Provides: rubygem-oj = %{version}
Provides: rubygem(oj) = %{version}
Obsoletes: rubygem-oj < %{version}
Obsoletes: rubygem(oj) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies oj -v %{version}
