...
Provides: rubygem(json) = %{version}
Obsoletes: rubygem-json < %{version}
Obsoletes: rubygem(json) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies json -v %{version}
