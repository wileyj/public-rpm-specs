...
Name: rubygem%{alinux_ruby}-recursive-open-struct
Version: 1.0.0
Release: 1.%{dist}
AutoReqProv : no
...
Provides: rubygem%{alinux_ruby}-recursive-open-struct = %{version}
Provides: rubygem%{alinux_ruby}(recursive-open-struct) = %{version}
Obsoletes: rubygem%{alinux_ruby}-recursive-open-struct < %{version}
Obsoletes: rubygem%{alinux_ruby}(recursive-open-struct) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --no-rdoc --no-ri --no-doc --ignore-dependencies recursive-open-struct -v %{version}
