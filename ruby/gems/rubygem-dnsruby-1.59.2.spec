...
Provides: rubygem-dnsruby = %{version}
Provides: rubygem(dnsruby) = %{version}
Obsoletes: rubygem-dnsruby < %{version}
Obsoletes: rubygem(dnsruby) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies dnsruby -v %{version}
