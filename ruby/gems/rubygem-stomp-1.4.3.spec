...
Provides: rubygem(stomp) = %{version}
Obsoletes: rubygem-stomp < %{version}
Obsoletes: rubygem(stomp) < %{version}
...
%description
Ruby client for the Stomp messaging protocol.  Note that this gem is no longer supported on rubyforge.

%prep
%build
%install
export CONFIGURE_ARGS="--with-cflags='%{optflags}'"
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies stomp -v %{version}
