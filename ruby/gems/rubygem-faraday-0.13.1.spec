...
Provides: rubygem-faraday = %{version}
Provides: rubygem(faraday) = %{version}
Obsoletes: rubygem-faraday < %{version}
Obsoletes: rubygem(faraday) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies faraday -v %{version}
