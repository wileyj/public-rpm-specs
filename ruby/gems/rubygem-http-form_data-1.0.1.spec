...
Version: 1.0.1
Release: 1.%{dist}
Summary: rubygem http-form_data
...
Provides: rubygem-http-form_data = %{version}
Provides: rubygem(http-form_data) = %{version}
Obsoletes: rubygem-http-form_data < %{version}
Obsoletes: rubygem(http-form_data) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies http-form_data -v %{version}
