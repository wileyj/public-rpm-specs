...
Provides: rubygem-ipaddress = %{version}
Provides: rubygem(ipaddress) = %{version}
Obsoletes: rubygem-ipaddress < %{version}
Obsoletes: rubygem(ipaddress) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies ipaddress -v %{version}
