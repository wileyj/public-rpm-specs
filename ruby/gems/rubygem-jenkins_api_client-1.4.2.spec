...
Name: rubygem-jenkins_api_client
Version: 1.4.2
Release: 1.%{dist}
...
Provides: rubygem-jenkins_api_client = %{version}
Provides: rubygem(jenkins_api_client) = %{version}
Obsoletes: rubygem-jenkins_api_client < %{version}
Obsoletes: rubygem(jenkins_api_client) < %{version}
...
Requires: rubygem-terminal-table  >= 1.4.0
Requires: rubygem-thor  >= 0.16.0
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies jenkins_api_client -v %{version}
