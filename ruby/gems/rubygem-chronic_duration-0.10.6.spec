...
%global gem_name chronic_duration
Name: rubygem-chronic_duration
Version: 0.10.6
Release: 1.%{dist}
Summary: rubygem chronic_duration
...
Provides: rubygem-chronic_duration = %{version}
Provides: rubygem(chronic_duration) = %{version}
Obsoletes: rubygem-chronic_duration < %{version}
Obsoletes: rubygem(chronic_duration) < %{version}


Requires: rubygem-numerizer  >= 0.1.1
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies chronic_duration -v %{version}
