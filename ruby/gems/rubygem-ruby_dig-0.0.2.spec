...
%global gem_name ruby_dig
Name: rubygem-ruby_dig
Version: 0.0.2
Release: 1.%{dist}
Summary: rubygem ruby_dig
...
Provides: rubygem-ruby_dig = %{version}
Provides: rubygem(ruby_dig) = %{version}
Obsoletes: rubygem-ruby_dig < %{version}
Obsoletes: rubygem(ruby_dig) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies ruby_dig -v %{version}
