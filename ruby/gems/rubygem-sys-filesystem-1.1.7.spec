...
Provides: rubygem-sys-filesystem = %{version}
Provides: rubygem(sys-filesystem) = %{version}
Obsoletes: rubygem-sys-filesystem < %{version}
Obsoletes: rubygem(sys-filesystem) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sys-filesystem -v %{version}
