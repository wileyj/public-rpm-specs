...
Provides: rubygem-docker-api = %{version}
Provides: rubygem(docker-api) = %{version}
Obsoletes: rubygem-docker-api < %{version}
Obsoletes: rubygem(docker-api) < %{version}


Requires: rubygem-excon  >= 0.38.0
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies docker-api -v %{version}
