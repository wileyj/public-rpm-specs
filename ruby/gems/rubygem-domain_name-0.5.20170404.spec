...
Provides: rubygem-domain_name = %{version}
Provides: rubygem(domain_name) = %{version}
Obsoletes: rubygem-domain_name < %{version}
Obsoletes: rubygem(domain_name) < %{version}


Requires: rubygem-unf  < 1.0.0,
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies domain_name -v %{version}
