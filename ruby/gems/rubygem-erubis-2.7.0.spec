...
Provides: rubygem(erubis) = %{version}
Obsoletes: rubygem-erubis < %{version}
Obsoletes: rubygem(erubis) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies erubis -v %{version}
