...
Provides: rubygem-terminal-table = %{version}
Provides: rubygem(terminal-table) = %{version}
Obsoletes: rubygem-terminal-table < %{version}
Obsoletes: rubygem(terminal-table) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies terminal-table -v %{version}
