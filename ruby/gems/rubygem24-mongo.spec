...
Name: rubygem%{alinux_ruby}-mongo
Version: 2.4.1
Release: 1.%{dist}
AutoReqProv : no
...
Provides: rubygem%{alinux_ruby}-mongo = %{version}
Provides: rubygem%{alinux_ruby}(mongo) = %{version}
Obsoletes: rubygem%{alinux_ruby}-mongo < %{version}
Obsoletes: rubygem%{alinux_ruby}(mongo) < %{version}


Requires: rubygem%{alinux_ruby}-bson  < 5.0.0,
