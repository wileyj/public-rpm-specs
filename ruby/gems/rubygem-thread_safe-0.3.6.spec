...
Provides: rubygem(thread_safe) = %{version}
Obsoletes: rubygem-thread_safe < %{version}
Obsoletes: rubygem(thread_safe) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies thread_safe -v %{version}
