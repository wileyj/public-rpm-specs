...
Provides: rubygem-unf_ext = %{version}
Provides: rubygem(unf_ext) = %{version}
Obsoletes: rubygem-unf_ext < %{version}
Obsoletes: rubygem(unf_ext) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies unf_ext -v %{version}
