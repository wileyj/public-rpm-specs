...
Name: rubygem-net_http_unix
Version: 0.2.2
Release: 1.%{dist}
Summary: rubygem net_http_unix
...
Provides: rubygem-net_http_unix = %{version}
Provides: rubygem(net_http_unix) = %{version}
Obsoletes: rubygem-net_http_unix < %{version}
Obsoletes: rubygem(net_http_unix) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies net_http_unix -v %{version}
