...
Provides: rubygem-addressable = %{version}
Provides: rubygem(addressable) = %{version}
Obsoletes: rubygem-addressable < %{version}
Obsoletes: rubygem(addressable) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies addressable -v %{version}
