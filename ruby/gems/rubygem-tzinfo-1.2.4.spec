...
Provides: rubygem(tzinfo) = %{version}
Obsoletes: rubygem-tzinfo < %{version}
Obsoletes: rubygem(tzinfo) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies tzinfo -v %{version}
