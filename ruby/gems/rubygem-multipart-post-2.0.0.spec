...
Provides: rubygem-multipart-post = %{version}
Provides: rubygem(multipart-post) = %{version}
Obsoletes: rubygem-multipart-post < %{version}
Obsoletes: rubygem(multipart-post) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies multipart-post -v %{version}
