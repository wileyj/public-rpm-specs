...
Name: rubygem-em-http-server
Version: 0.1.8
Release: 1.%{dist}
Summary: rubygem em-http-server
...
Provides: rubygem-em-http-server = %{version}
Provides: rubygem(em-http-server) = %{version}
Obsoletes: rubygem-em-http-server < %{version}
Obsoletes: rubygem(em-http-server) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies em-http-server -v %{version}
