...
Provides: rubygem-bunny = %{version}
Provides: rubygem(bunny) = %{version}
Obsoletes: rubygem-bunny < %{version}
Obsoletes: rubygem(bunny) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies bunny -v %{version}
