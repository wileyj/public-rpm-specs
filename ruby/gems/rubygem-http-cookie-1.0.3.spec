...
Provides: rubygem-http-cookie = %{version}
Provides: rubygem(http-cookie) = %{version}
Obsoletes: rubygem-http-cookie < %{version}
Obsoletes: rubygem(http-cookie) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies http-cookie -v %{version}
