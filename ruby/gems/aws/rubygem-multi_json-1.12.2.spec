...
Provides: rubygem(multi_json) = %{version}
Obsoletes: rubygem-multi_json < %{version}
Obsoletes: rubygem(multi_json) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies multi_json -v %{version}
