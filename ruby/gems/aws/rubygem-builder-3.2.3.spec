...
Provides: rubygem(builder) = %{version}
Obsoletes: rubygem-builder < %{version}
Obsoletes: rubygem(builder) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies builder -v %{version}
