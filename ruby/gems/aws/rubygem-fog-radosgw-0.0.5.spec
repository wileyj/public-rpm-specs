...
Name: rubygem-fog-radosgw
Version: 0.0.5
Release: 1.%{dist}
Summary: rubygem fog-radosgw
...
Provides: rubygem-fog-radosgw = %{version}
Provides: rubygem(fog-radosgw) = %{version}
Obsoletes: rubygem-fog-radosgw < %{version}
Obsoletes: rubygem(fog-radosgw) < %{version}


Requires: rubygem-fog-core   >= 1.21.0
Requires: rubygem-fog-json 
Requires: rubygem-fog-xml   >= 0.0.1
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-radosgw -v %{version}
(
    cd %{buildroot}
