...
Name: rubygem-fog-brightbox
Version: 0.14.0
Release: 1.%{dist}
Summary: rubygem fog-brightbox
...
Provides: rubygem-fog-brightbox = %{version}
Provides: rubygem(fog-brightbox) = %{version}
Obsoletes: rubygem-fog-brightbox < %{version}
Obsoletes: rubygem(fog-brightbox) < %{version}


Requires: rubygem-fog-core  = 1.22
Requires: rubygem-fog-json 
Requires: rubygem-inflecto  = 0.0.2
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-brightbox -v %{version}
