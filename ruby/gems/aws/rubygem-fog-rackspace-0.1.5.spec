...
Name: rubygem-fog-rackspace
Version: 0.1.5
Release: 1.%{dist}
Summary: rubygem fog-rackspace
...
Provides: rubygem-fog-rackspace = %{version}
Provides: rubygem(fog-rackspace) = %{version}
Obsoletes: rubygem-fog-rackspace < %{version}
Obsoletes: rubygem(fog-rackspace) < %{version}


Requires: rubygem-fog-core   >= 1.35
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-rackspace -v %{version}
(
    cd %{buildroot}
