...
Name: rubygem-fog-ovirt
Version: 0.1.1
Release: 1.%{dist}
Summary: rubygem fog-ovirt
...
Provides: rubygem-fog-ovirt = %{version}
Provides: rubygem(fog-ovirt) = %{version}
Obsoletes: rubygem-fog-ovirt < %{version}
Obsoletes: rubygem(fog-ovirt) < %{version}
...
Requires: rubygem-rbovirt   = 0.1.4
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-ovirt -v %{version}
(
    cd %{buildroot}
