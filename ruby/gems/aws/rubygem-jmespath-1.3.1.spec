...
Provides: rubygem-jmespath = %{version}
Provides: rubygem(jmespath) = %{version}
Obsoletes: rubygem-jmespath < %{version}
Obsoletes: rubygem(jmespath) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies jmespath -v %{version}
