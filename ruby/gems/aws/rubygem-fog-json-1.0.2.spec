...
Provides: rubygem-fog-json = %{version}
Provides: rubygem(fog-json) = %{version}
Obsoletes: rubygem-fog-json < %{version}
Obsoletes: rubygem(fog-json) < %{version}


Requires: rubygem-fog-core  = 1.0
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-json -v %{version}
