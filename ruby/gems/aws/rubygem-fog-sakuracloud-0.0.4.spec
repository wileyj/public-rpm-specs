...
Name: rubygem-fog-sakuracloud
Version: 0.0.4
Release: 1.%{dist}
Summary: rubygem fog-sakuracloud
...
Provides: rubygem-fog-sakuracloud = %{version}
Provides: rubygem(fog-sakuracloud) = %{version}
Obsoletes: rubygem-fog-sakuracloud < %{version}
Obsoletes: rubygem(fog-sakuracloud) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-sakuracloud -v %{version}
