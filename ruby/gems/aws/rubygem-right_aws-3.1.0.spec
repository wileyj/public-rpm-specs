...
Provides: rubygem(right_aws) = %{version}
Obsoletes: rubygem-right_aws < %{version}
Obsoletes: rubygem(right_aws) < %{version}


Requires: rubygem-right_http_connection  >= 1.2.5
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies right_aws -v %{version}
