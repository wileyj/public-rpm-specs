...
Provides: rubygem(xml-simple) = %{version}
Obsoletes: rubygem-xml-simple < %{version}
Obsoletes: rubygem(xml-simple) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies xml-simple -v %{version}
(
    cd %{buildroot}
