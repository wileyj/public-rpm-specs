...
Name: rubygem-fog-softlayer
Version: 1.1.4
Release: 1.%{dist}
Summary: rubygem fog-softlayer
...
Provides: rubygem-fog-softlayer = %{version}
Provides: rubygem(fog-softlayer) = %{version}
Obsoletes: rubygem-fog-softlayer < %{version}
Obsoletes: rubygem(fog-softlayer) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-softlayer -v %{version}
