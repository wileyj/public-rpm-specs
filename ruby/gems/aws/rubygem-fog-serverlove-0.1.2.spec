...
Name: rubygem-fog-serverlove
Version: 0.1.2
Release: 1.%{dist}
Summary: rubygem fog-serverlove
...
Provides: rubygem-fog-serverlove = %{version}
Provides: rubygem(fog-serverlove) = %{version}
Obsoletes: rubygem-fog-serverlove < %{version}
Obsoletes: rubygem(fog-serverlove) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-serverlove -v %{version}
