...
Name: rubygem-CFPropertyList
Version: 2.3.5
Release: 1.%{dist}
Summary: rubygem CFPropertyList
...
Provides: rubygem-CFPropertyList = %{version}
Provides: rubygem(CFPropertyList) = %{version}
Obsoletes: rubygem-CFPropertyList < %{version}
Obsoletes: rubygem(CFPropertyList) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies CFPropertyList -v %{version}
