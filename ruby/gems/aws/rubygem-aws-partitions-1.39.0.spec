...
Provides: rubygem-aws-partitions = %{version}
Provides: rubygem(aws-partitions) = %{version}
Obsoletes: rubygem-aws-partitions < %{version}
Obsoletes: rubygem(aws-partitions) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies aws-partitions -v %{version}
