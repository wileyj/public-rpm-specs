...
%global gem_name fog-cloudatcost
Name: rubygem-fog-cloudatcost
Version: 0.2.3
Release: 1.%{dist}
Summary: rubygem fog-cloudatcost
Group: Development/Languages
License: MIT
URL: http://github.com/fog/fog-cloudatcost
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-fog-cloudatcost = %{version}
Provides: rubygem(fog-cloudatcost) = %{version}
Obsoletes: rubygem-fog-cloudatcost < %{version}
Obsoletes: rubygem(fog-cloudatcost) < %{version}


Requires: rubygem-fog-core   = 1.41
Requires: rubygem-fog-json   = 1.0
Requires: rubygem-fog-xml   = 0.1
Requires: rubygem-ipaddress   = 0.8
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-cloudatcost -v %{version}
(
    cd %{buildroot}
