...
Name: rubygem-fog-atmos
Version: 0.1.0
Release: 1.%{dist}
Summary: rubygem fog-atmos
...
Provides: rubygem-fog-atmos = %{version}
Provides: rubygem(fog-atmos) = %{version}
Obsoletes: rubygem-fog-atmos < %{version}
Obsoletes: rubygem(fog-atmos) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-atmos -v %{version}
