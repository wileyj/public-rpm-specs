...
Provides: rubygem(rbvmomi) = %{version}
Obsoletes: rubygem-rbvmomi < %{version}
Obsoletes: rubygem(rbvmomi) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies rbvmomi -v %{version}
(
    cd %{buildroot}
