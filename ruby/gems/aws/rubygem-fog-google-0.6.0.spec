...
Name: rubygem-fog-google
Version: 0.6.0
Release: 1.%{dist}
Summary: rubygem fog-google
...
Provides: rubygem-fog-google = %{version}
Provides: rubygem(fog-google) = %{version}
Obsoletes: rubygem-fog-google < %{version}
Obsoletes: rubygem(fog-google) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-google -v %{version}
(
    cd %{buildroot}
