...
Name: rubygem-fog-internet-archive
Version: 0.0.1
Release: 1.%{dist}
Summary: rubygem fog-internet-archive
...
Provides: rubygem-fog-internet-archive = %{version}
Provides: rubygem(fog-internet-archive) = %{version}
Obsoletes: rubygem-fog-internet-archive < %{version}
Obsoletes: rubygem(fog-internet-archive) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-internet-archive -v %{version}
(
    cd %{buildroot}
