...
%global gem_name fog-dnsimple
Name: rubygem-fog-dnsimple
Version: 1.0.0
Release: 1.%{dist}
Summary: rubygem fog-dnsimple
...
Provides: rubygem-fog-dnsimple = %{version}
Provides: rubygem(fog-dnsimple) = %{version}
Obsoletes: rubygem-fog-dnsimple < %{version}
Obsoletes: rubygem(fog-dnsimple) < %{version}


Requires: rubygem-fog-core   = 1.38
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-dnsimple -v %{version}
(
    cd %{buildroot}
