...
Provides: rubygem-fog-xml = %{version}
Provides: rubygem(fog-xml) = %{version}
Obsoletes: rubygem-fog-xml < %{version}
Obsoletes: rubygem(fog-xml) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-xml -v %{version}
