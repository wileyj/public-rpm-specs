...
Name: rubygem-fog-digitalocean
Version: 0.3.0
Release: 1.%{dist}
Summary: rubygem fog-digitalocean
...
Provides: rubygem-fog-digitalocean = %{version}
Provides: rubygem(fog-digitalocean) = %{version}
Obsoletes: rubygem-fog-digitalocean < %{version}
Obsoletes: rubygem(fog-digitalocean) < %{version}


Requires: rubygem-fog-core   = 1.42
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-digitalocean -v %{version}
(
    cd %{buildroot}
