...
Name: rubygem-fog-terremark
Version: 0.1.0
Release: 1.%{dist}
Summary: rubygem fog-terremark
...
Provides: rubygem-fog-terremark = %{version}
Provides: rubygem(fog-terremark) = %{version}
Obsoletes: rubygem-fog-terremark < %{version}
Obsoletes: rubygem(fog-terremark) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-terremark -v %{version}
