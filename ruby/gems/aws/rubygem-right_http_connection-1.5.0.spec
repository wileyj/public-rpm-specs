...
%global gem_name right_http_connection
Name: rubygem-right_http_connection
Version: 1.5.0
Release: 1.%{dist}
Summary: rubygem right_http_connection
...
Provides: rubygem(right_http_connection) = %{version}
Obsoletes: rubygem-right_http_connection < %{version}
Obsoletes: rubygem(right_http_connection) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies right_http_connection -v %{version}
