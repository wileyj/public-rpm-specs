...
Name: rubygem-inflecto
Version: 0.0.2
Release: 1.%{dist}
Summary: rubygem inflecto
...
Provides: rubygem-inflecto = %{version}
Provides: rubygem(inflecto) = %{version}
Obsoletes: rubygem-inflecto < %{version}
Obsoletes: rubygem(inflecto) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies inflecto -v %{version}
