...
Name: rubygem-fog-ecloud
Version: 0.3.0
Release: 1.%{dist}
Summary: rubygem fog-ecloud
...
Provides: rubygem-fog-ecloud = %{version}
Provides: rubygem(fog-ecloud) = %{version}
Obsoletes: rubygem-fog-ecloud < %{version}
Obsoletes: rubygem(fog-ecloud) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-ecloud -v %{version}
(
    cd %{buildroot}
