...
%global gem_name fog-aliyun
Name: rubygem-fog-aliyun
Version: 0.2.0
Release: 1.%{dist}
Summary: rubygem fog-aliyun
...
Provides: rubygem-fog-aliyun = %{version}
Provides: rubygem(fog-aliyun) = %{version}
Obsoletes: rubygem-fog-aliyun < %{version}
Obsoletes: rubygem(fog-aliyun) < %{version}
...
Requires: rubygem-xml-simple   = 1.1
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-aliyun -v %{version}
(
    cd %{buildroot}
