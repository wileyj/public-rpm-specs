...
Provides: rubygem-fog-aws = %{version}
Provides: rubygem(fog-aws) = %{version}
Obsoletes: rubygem-fog-aws < %{version}
Obsoletes: rubygem(fog-aws) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-aws -v %{version}
(
    cd %{buildroot}
