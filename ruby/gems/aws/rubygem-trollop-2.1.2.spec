...
Provides: rubygem(trollop) = %{version}
Obsoletes: rubygem-trollop < %{version}
Obsoletes: rubygem(trollop) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies trollop -v %{version}
(
    cd %{buildroot}
