...
%global gem_name fog-local
Name: rubygem-fog-local
Version: 0.4.0
Release: 1.%{dist}
Summary: rubygem fog-local
...
Provides: rubygem-fog-local = %{version}
Provides: rubygem(fog-local) = %{version}
Obsoletes: rubygem-fog-local < %{version}
Obsoletes: rubygem(fog-local) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-local -v %{version}
