...
%global gem_name fog-dynect
Name: rubygem-fog-dynect
Version: 0.2.0
Release: 1.%{dist}
Summary: rubygem fog-dynect
...
Provides: rubygem-fog-dynect = %{version}
Provides: rubygem(fog-dynect) = %{version}
Obsoletes: rubygem-fog-dynect < %{version}
Obsoletes: rubygem(fog-dynect) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-dynect -v %{version}
(
    cd %{buildroot}
