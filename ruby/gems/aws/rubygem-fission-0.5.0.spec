...
Name: rubygem-fission
Version: 0.5.0
Release: 1.%{dist}
Summary: rubygem fission
...
Provides: rubygem-fission = %{version}
Provides: rubygem(fission) = %{version}
Obsoletes: rubygem-fission < %{version}
Obsoletes: rubygem(fission) < %{version}


Requires: rubygem-CFPropertyList  = 2.2
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fission -v %{version}
