...
Name: rubygem-fog-openstack
Version: 0.1.22
Release: 1.%{dist}
Summary: rubygem fog-openstack
...
Provides: rubygem-fog-openstack = %{version}
Provides: rubygem(fog-openstack) = %{version}
Obsoletes: rubygem-fog-openstack < %{version}
Obsoletes: rubygem(fog-openstack) < %{version}


Requires: rubygem-fog-core   >= 1.40
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-openstack -v %{version}
(
    cd %{buildroot}
