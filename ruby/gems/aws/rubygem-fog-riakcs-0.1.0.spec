...
Name: rubygem-fog-riakcs
Version: 0.1.0
Release: 1.%{dist}
Summary: rubygem fog-riakcs
...
Provides: rubygem-fog-riakcs = %{version}
Provides: rubygem(fog-riakcs) = %{version}
Obsoletes: rubygem-fog-riakcs < %{version}
Obsoletes: rubygem(fog-riakcs) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-riakcs -v %{version}
