...
Name: rubygem-fog-xenserver
Version: 0.3.0
Release: 1.%{dist}
Summary: rubygem fog-xenserver
...
Provides: rubygem-fog-xenserver = %{version}
Provides: rubygem(fog-xenserver) = %{version}
Obsoletes: rubygem-fog-xenserver < %{version}
Obsoletes: rubygem(fog-xenserver) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-xenserver -v %{version}
(
    cd %{buildroot}
