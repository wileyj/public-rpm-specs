...
Provides: rubygem(excon) = %{version}
Obsoletes: rubygem-excon < %{version}
Obsoletes: rubygem(excon) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies excon -v %{version}
