...
Name: rubygem-fog-voxel
Version: 0.1.0
Release: 1.%{dist}
Summary: rubygem fog-voxel
...
Provides: rubygem-fog-voxel = %{version}
Provides: rubygem(fog-voxel) = %{version}
Obsoletes: rubygem-fog-voxel < %{version}
Obsoletes: rubygem(fog-voxel) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-voxel -v %{version}
