...
Name: rubygem-fog-vmfusion
Version: 0.1.0
Release: 1.%{dist}
Summary: rubygem fog-vmfusion
...
Provides: rubygem-fog-vmfusion = %{version}
Provides: rubygem(fog-vmfusion) = %{version}
Obsoletes: rubygem-fog-vmfusion < %{version}
Obsoletes: rubygem(fog-vmfusion) < %{version}


Requires: rubygem-fission 
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-vmfusion -v %{version}
