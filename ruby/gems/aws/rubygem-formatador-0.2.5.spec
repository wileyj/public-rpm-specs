...
Provides: rubygem(formatador) = %{version}
Obsoletes: rubygem-formatador < %{version}
Obsoletes: rubygem(formatador) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies formatador -v %{version}
