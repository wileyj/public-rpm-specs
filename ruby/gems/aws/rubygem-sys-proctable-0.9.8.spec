...
Name: rubygem-sys-proctable
Version: 0.9.8
Release: 1.%{dist}
Summary: rubygem sys-proctable
...
Provides: rubygem-sys-proctable = %{version}
Provides: rubygem(sys-proctable) = %{version}
Obsoletes: rubygem-sys-proctable < %{version}
Obsoletes: rubygem(sys-proctable) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies sys-proctable -v %{version}
(
    cd %{buildroot}
