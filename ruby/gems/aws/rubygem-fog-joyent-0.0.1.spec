...
%global gem_name fog-joyent
Name: rubygem-fog-joyent
Version: 0.0.1
Release: 1.%{dist}
Summary: rubygem fog-joyent
...
Provides: rubygem-fog-joyent = %{version}
Provides: rubygem(fog-joyent) = %{version}
Obsoletes: rubygem-fog-joyent < %{version}
Obsoletes: rubygem(fog-joyent) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies fog-joyent -v %{version}
(
    cd %{buildroot}
