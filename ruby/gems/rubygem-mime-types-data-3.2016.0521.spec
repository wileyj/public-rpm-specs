...
Provides: rubygem-mime-types-data = %{version}
Provides: rubygem(mime-types-data) = %{version}
Obsoletes: rubygem-mime-types-data < %{version}
Obsoletes: rubygem(mime-types-data) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies mime-types-data -v %{version}
