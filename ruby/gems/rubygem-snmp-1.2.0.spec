...
Provides: rubygem-snmp = %{version}
Provides: rubygem(snmp) = %{version}
Obsoletes: rubygem-snmp < %{version}
Obsoletes: rubygem(snmp) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies snmp -v %{version}
