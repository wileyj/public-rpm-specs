...
then
    ...
    sed -i -e 's|", "[0-9].*|"|g' %{buildroot}%{gem_dir}/specifications/%{gem_name}-%{version}.gemspec
     sed -i -e '\|%q<json>.freez|d'  %{buildroot}%{gem_dir}/specifications/%{gem_name}-%{version}.gemspec
fi
...
then
    ...
    sed -i -e 's|", "[0-9].*|"|g'  %{buildroot}%{gem_dir}/gems/%{gem_name}-%{version}/%{gem_name}.gemspec
     sed -i -e '\|%q<json>.freez|d'   %{buildroot}%{gem_dir}/gems/%{gem_name}-%{version}/%{gem_name}.gemspec
fi
