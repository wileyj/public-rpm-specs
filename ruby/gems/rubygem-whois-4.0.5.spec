...
%global gem_name whois
Name: rubygem-whois
Version: 4.0.5
Release: 1.%{dist}
Summary: rubygem whois
...
Provides: rubygem-whois = %{version}
Provides: rubygem(whois) = %{version}
Obsoletes: rubygem-whois < %{version}
Obsoletes: rubygem(whois) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies whois -v %{version}
