...
Provides: rubygem(thor) = %{version}
Obsoletes: rubygem-thor < %{version}
Obsoletes: rubygem(thor) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies thor -v %{version}
