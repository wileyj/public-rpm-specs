...
Provides: rubygem(minitest) = %{version}
Obsoletes: rubygem-minitest < %{version}
Obsoletes: rubygem(minitest) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies minitest -v %{version}
