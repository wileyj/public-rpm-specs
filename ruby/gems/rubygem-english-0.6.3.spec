...
%global gem_name english
Name: rubygem-english
Version: 0.6.3
Release: 1.%{dist}
Summary: rubygem english
Group: Development/Languages
License: GPL
URL: http://rubyworks.github.com/english
Vendor: %{vendor}
Packager: %{packager}
Requires: ruby rubygems
Provides: rubygem-english = %{version}
Provides: rubygem(english) = %{version}
Obsoletes: rubygem-english < %{version}
Obsoletes: rubygem(english) < %{version}


Requires: rubygem-language 


%description
English is an general purpose English language processing library. It includes a number of useful libraries, such as inflect.rb, metaphone.rb, ordinal.rb, and so on.

%prep
%build
%install
export CONFIGURE_ARGS="--with-cflags='%{optflags}'"
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies english -v %{version}
