...
Name: rubygem-parse-cron
Version: 0.1.4
Release: 1.%{dist}
Summary: rubygem parse-cron
...
Provides: rubygem-parse-cron = %{version}
Provides: rubygem(parse-cron) = %{version}
Obsoletes: rubygem-parse-cron < %{version}
Obsoletes: rubygem(parse-cron) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies parse-cron -v %{version}
