...
Provides: rubygem(nokogiri) = %{version}
Obsoletes: rubygem-nokogiri < %{version}
Obsoletes: rubygem(nokogiri) < %{version}


Requires: rubygem-mini_portile  >= 0.5.0
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies nokogiri -v %{version}
