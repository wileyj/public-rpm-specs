...
Name: rubygem-em-worker
Version: 0.0.2
Release: 1.%{dist}
Summary: rubygem em-worker
...
Provides: rubygem-em-worker = %{version}
Provides: rubygem(em-worker) = %{version}
Obsoletes: rubygem-em-worker < %{version}
Obsoletes: rubygem(em-worker) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies em-worker -v %{version}
