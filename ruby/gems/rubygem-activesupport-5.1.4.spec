...
Provides: rubygem(activesupport) = %{version}
Obsoletes: rubygem-activesupport < %{version}
Obsoletes: rubygem(activesupport) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies activesupport -v %{version}
