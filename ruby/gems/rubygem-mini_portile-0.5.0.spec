...
Provides: rubygem-mini_portile = %{version}
Provides: rubygem(mini_portile) = %{version}
Obsoletes: rubygem-mini_portile < %{version}
Obsoletes: rubygem(mini_portile) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies mini_portile -v %{version}
