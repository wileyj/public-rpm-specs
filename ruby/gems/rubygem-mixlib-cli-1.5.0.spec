...
Provides: rubygem-mixlib-cli = %{version}
Provides: rubygem(mixlib-cli) = %{version}
Obsoletes: rubygem-mixlib-cli < %{version}
Obsoletes: rubygem(mixlib-cli) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies mixlib-cli -v %{version}
