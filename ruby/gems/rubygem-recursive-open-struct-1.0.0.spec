...
Provides: rubygem-recursive-open-struct = %{version}
Provides: rubygem(recursive-open-struct) = %{version}
Obsoletes: rubygem-recursive-open-struct < %{version}
Obsoletes: rubygem(recursive-open-struct) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies recursive-open-struct -v %{version}
