...
Provides: rubygem(rest-client) = %{version}
Obsoletes: rubygem-rest-client < %{version}
Obsoletes: rubygem(rest-client) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies rest-client -v %{version}
