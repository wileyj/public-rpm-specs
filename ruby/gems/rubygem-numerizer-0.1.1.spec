...
%global gem_name numerizer
Name: rubygem-numerizer
Version: 0.1.1
Release: 1.%{dist}
Summary: rubygem numerizer
...
Provides: rubygem-numerizer = %{version}
Provides: rubygem(numerizer) = %{version}
Obsoletes: rubygem-numerizer < %{version}
Obsoletes: rubygem(numerizer) < %{version}
...
gem install --install-dir %{buildroot}/%{gem_dir} --bindir %{buildroot}/%{_bindir} --force --no-rdoc --no-ri --no-doc --ignore-dependencies numerizer -v %{version}
